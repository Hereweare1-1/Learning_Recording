...
- 还有统计出现元素个数的count(元素)方法
- 除此之外，还有其他通用操作，比如——
	min(s)：获取最小值；max(s)：获取最大值；sum(s)：求和；len(s)：获取序列长度
### 案例
- **合并两列表并进行去重**
```python
...
```
- 合并2个列表的方法
1. 遍历其中一个列表并把元素逐个添加到另一个列表（如上代码）(不推荐)
2. 使用+直接合并
```python
...
- 定义：字典名称={key: value, key:value, key:value ... }
	定义空字典：字典名称={} 或 字典名称=dict()
- 特点：键值对(key:value)存储、键(key)不能重复、可修改。
- 注意：
	value可以是任意类型,而key必须是不可变类型(不能为list、set、dict)
	字典内的key不允许重复,如果重复定义,否则后面value的会覆盖前面的
	字典是没有索引下标的,不能根据索引获取值,只可以根据key获取value
### 方法
![file-20260907103659181](assets/数据存储容器/file-20260907103659181.png)
## 总结
![file-20260907122800654](assets/数据存储容器/file-20260907122800654.png)
